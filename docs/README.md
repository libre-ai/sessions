...
> gate-wall as it stood then (cargo-deny + cargo-audit + guard-scan + coverage ≥ 80%);
> of that wall, cargo-deny and cargo-audit run today in `.github/workflows/rust.yml`
> and the guard-scan invariants in `context-hygiene.yml`, while **no coverage gate
> exists in this repository** — the 80% figure is a historical measurement, not an
> enforced threshold. The §3
...
- **Sovereignty.** OSS licensing only (MIT/Apache/MPL family, enforced by `cargo deny check licenses` against `deny.toml` in the `Supply-chain policy` job; `cargo-audit` runs alongside it but reports rather than blocks), EU residency, no US hyperscaler/gatekeeper, and no internal/proprietary-employer reference of any kind in any artifact (this is a clean-room sovereign OSS repo).
